...
private:
    ...
    void set_namelist(char* buf, int& offset, vector<const char*>& namev);
...
public:
    ...
    int send_file(const char* s, int len, bool block, vector<const char*>& namev); 
    int send_msg(const char*s, int len, bool block, vector<const char*>& namev);
